        ...
        <div className="px-10 md:px-24 lg:px-36 xl:px-50 2xl:px-64 w-full flex flex-col items-center gap-y-[4vh] md:gap-y-[8vh]">
          <div className="grid grid-cols-2 md:flex md:flex-row gap-2 md:gap-x-24">
            {CP_ACCOUNTS.map((account, index) => (
              <a
                aria-label={`${account.platform} Link`}
                key={index}
                className={`${index == 0 && "col-span-2 justify-self-center"}`}
                href={account.link}
                target="_blank">
                <div className="py-4 md:py-6 w-32 md:w-42 bg-secondary-100 border-2 border-secondary-500 rounded-lg flex flex-col gap-y-2 md:gap-y-4 items-center justify-center cursor-pointer hover:scale-110 transition-transform duration-300">
                  <img
          ...
          <div className="w-full grid grid-cols-1 md:grid-cols-2 h-full justify-between gap-4 md:gap-8">
            {ACHIEVEMENTS.map((achievement, index) => (
              <div
                key={index}
                className="flex h-[8vh] md:h-[10vh] py-2 md:py-4 px-6 bg-secondary-100 border-2 rounded-xl border-secondary-500 items-center justify-between gap-x-4">
        ...
        <div className="px-10 md:px-24 lg:px-36 xl:px-50 2xl:px-64 w-full flex justify-center items-center gap-x-12 md:pt-[4vh]">
          {SOCIALS.map((social, index) => (
            <a
              ...
              href={social.link}
              target="_blank">
              <div className="h-10 w-10 xl:h-14 xl:w-14 flex items-center justify-center bg-secondary-100 border-2 border-secondary-500 text-tertiary hover:text-primary hover:bg-secondary rounded-full transition-all duration-300">
                {social.icon}
              </div>
            </a>
          ))}
